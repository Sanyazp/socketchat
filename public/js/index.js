...
function addMessageOnPage(data, who) {
    ...
    if (who == 'me') {
        ...
        document.querySelector(`.${data.to}`).innerHTML += html;
    } else {
        if (!data.private) {
            html = `
            <div class="messageInChat">
                ...
                `
            document.querySelector(`.All_Users`).innerHTML += html;
        } else {
            html = `
            <div class="messageInChat">
                ...
                `
            document.querySelector(`.${data.username}`).innerHTML += html;
        }
...
function usersDialogsUnHider(user) {
    ...
    for (var i = 0; i < parent.children.length; i++) {
        ...
        if (parent.children[i].classList.contains(user)) {
